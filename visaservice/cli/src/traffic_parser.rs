...
pub fn parse_traffic(input: &str, prot: Protocol) -> Result<vsapi::TrafficDesc, std::io::Error> {
    ...
    if input.starts_with("[") { 
        // IPv6
        let re = Regex::new(r"\[([0-9a-fA-F:]+)\](?::(\d+))?>\[([0-9a-fA-F:]+)\]:(\d+)(?:\[([SA]+)\])?").unwrap();
        capts = match re.captures(input) {
            Some(caps) => caps,
            None => return Err(std::io::Error::new(std::io::ErrorKind::Other, "Invalid input"))
        };
    } else {
        // IPv4
    ...
    let src_addr = match capts.get(1).unwrap().as_str().parse::<IpAddr>() {
        Ok(addr) => addr,
        Err(_) => return Err(std::io::Error::new(std::io::ErrorKind::Other, "Invalid source address"))
    };
    let dst_addr = match capts.get(3).unwrap().as_str().parse::<IpAddr>() {
        Ok(addr) => addr,
        Err(_) => return Err(std::io::Error::new(std::io::ErrorKind::Other, "Invalid destination address"))
    };

    if (src_addr.is_ipv4() && dst_addr.is_ipv6()) || (src_addr.is_ipv6() && dst_addr.is_ipv4()) {   
        return Err(std::io::Error::new(std::io::ErrorKind::Other, "Cannot mix IPv4 and IPv6 addresses"));
    }
    ...
    let src_port: u16 = match capts.get(2) {
        Some(port) => match port.as_str().parse::<u16>() {
            Ok(p) => p,
            Err(_) => return Err(std::io::Error::new(std::io::ErrorKind::Other, "Invalid source port"))
        },
        None => { rng.gen() }
    };
    ...
    let dst_port: u16 = match capts.get(4).unwrap().as_str().parse::<u16>() {
        Ok(port) => port,
        Err(_) => return Err(std::io::Error::new(std::io::ErrorKind::Other, "Invalid destination port"))
    };
        ...
        Some(fstr) => {
            for c in fstr.as_str().chars() {
                match c {
                    ...
                    _ => return Err(std::io::Error::new(std::io::ErrorKind::Other, "Invalid flags"))
                }
            }
        },
        None => {}
    ...
    if flags > 0 && prot != Protocol::TCP {
        return Err(std::io::Error::new(std::io::ErrorKind::Other, "Flags only valid for TCP"));
    }
