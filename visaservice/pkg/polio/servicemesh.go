...
func NewServiceMeshFromPolicy(policy *Policy) *ServiceMesh {
	mesh := NewServiceMesh()
	...
	for _, authsvc := range policy.GetServices() {
		...
		mesh.services[skey] = append(mesh.services[skey], &ServiceRec{
			...
			name:     authsvc.Name,
			port:     portn,
			protocol: AuthProtocol.String(),
			stype:    SvcT_SVCT_AUTH,
		})
	}
	...
	for _, cpos := range policy.GetConnects() {
		...
		for _, ins := range proc.Proc {
			if ins.Opcode == OpCodeT_OP_Register {
...
func (mesh *ServiceMesh) AddService(proto snip.Protocol, port uint16, address string, name string, svctype SvcT) {
	svc := &ServiceRec{
		address:  address,
		name:     name,
		port:     int(port),
...
func (mesh *ServiceMesh) AddServiceWithConstraints(proto snip.Protocol, port uint16, address string, name string, svctype SvcT, constraints []string) {
