...
use serde_json::json;
use hmac::{Hmac, Mac};
use jwt::{AlgorithmType, Header, Claims, SignWithKey, Token};
use sha2::Sha384;
...
pub fn create_token(client_id: &str, attributes: &Vec<(String, String)>) -> String {
    ...
    token_claims.registered.issued_at = Some(SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs());
    token_claims.registered.expiration = Some(SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs() + JWT_LIFETIME_SECONDS);
    ...
    for tuple in attributes {
        token_claims.private.insert(format!("zpra/{}", tuple.0), json!(tuple.1.clone()));
    }
    ...
    let token = Token::new(header, token_claims).sign_with_key(&key).unwrap();
...
pub fn claims_for(tstr: &str) -> Result<BTreeMap<String, String>, jwt::Error> {
    ...
    if claims.registered.issued_at.is_some(){
        result.insert("iat".to_string(), claims.registered.issued_at.unwrap().to_string());
    }
    if claims.registered.expiration.is_some(){
        result.insert("exp".to_string(), claims.registered.expiration.unwrap().to_string());
    }
    if claims.registered.not_before.is_some(){
        result.insert("nbf".to_string(), claims.registered.not_before.unwrap().to_string());
    }
    if claims.registered.json_web_token_id.is_some(){
        result.insert("jti".to_string(), claims.registered.json_web_token_id.unwrap());
    }
