...
```bash
zpc -k zpr-rsa-key.pem v4-1node-2agent-ping.zpl
zpc -k zpr-rsa-key.pem v4-1node-3agent-ping.zpl
```
...
```bash
zpc -c v6-1node-2agent-ping.zplc -k zpr-rsa-key.pem -o v6-1node-2agent-ping.bin v4-1node-2agent-ping.zpl
zpc -c v6-1node-3agent-ping.zplc -k zpr-rsa-key.pem -o v6-1node-3agent-ping.bin v4-1node-3agent-ping.zpl
```
