...
An agent initiates a flow by sending a packet to the Ingress Client Adapter, the gateway into the ZPR network.  At this point, the Adapter has no entry for the flow in its Agent Lookup Table (keyed by the 5-tuple) and the failure of that lookup will indicate that it needs to issue a ZDP Bind Request to the dock.  A Bind Request is created and the packet is inserted into the Bind Request and it is forwarded to the Dock.
