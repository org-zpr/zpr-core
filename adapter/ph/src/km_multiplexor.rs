...
impl<'pktbuf> KmState<'pktbuf> {
    pub fn new(km_buffers_out: mpsc::Sender<KMLinkMsg<Bytes>>, km_sig_tx: mpsc::Sender<KMLinkMsg<KMSignal>>, ctok: CancellationToken) -> Self {
        Self {
            ctok,
    ...
    pub fn add_link_handle(&self, link_id: zpr::LinkId, handle: KMHandle<'pktbuf>) {
...
pub struct SAState {
    ...
    /// TODO: We could keep the km::Codec here and not have encrypt/decrypt functons on KM at all.
    ///
...
/// SAState is Security Association State and is kept per peer link.
impl SAState {
...
/// Creates a new KeyManager for the link and starts its state machine.
pub fn add_adapter_link (
    ...
    ) -> Result<(), String>
    {
        ...
        let noise = match KMNoise::new(true, Some(peer_noise_key.into()), None, recv_zpis.encr, recv_zpis.hmac) {
            Ok(n) => n,
            Err(e) => {
        ...
        asm.sa_states.lock().unwrap().insert(link_id, SAState::new());
