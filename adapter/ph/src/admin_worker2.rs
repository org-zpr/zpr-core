...
impl cli::cmd_line_inter::Server for AdminServiceImpl {

}

pub async fn launch_capnp(_asm: Arc<Assembly>) -> Result<(), Box<dyn std::error::Error>> {
    let listener = tokio::net::TcpListener::bind(("localhost", 12345)).await?;
    println!("after bind");
    ...
    tokio::task::LocalSet::new()
        .run_until(async move {
            loop {
                ...
                let _network = capnp_rpc::twoparty::VatNetwork::new(
                ...
                //     println!("[{session_id}] Disconnect from {addr}: {err:?}");
                //     err
                // });
            }
        })
...
pub async fn launch(asm: Arc<Assembly>, _socket: Arc<UnixListener>) {
    match launch_capnp(asm.clone()).await {
        Ok(()) => println!("SUCCESS"),
