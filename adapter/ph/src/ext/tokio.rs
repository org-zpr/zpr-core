pub mod io {
    pub mod unix {
        ...
        pub async fn async_fd_read<T: AsFd + AsRawFd>(
            afd: &AsyncFd<T>,
            buf: &mut [u8],
        ) -> io::Result<usize> {
            afd.async_io(Interest::READABLE, |fd| {
                nix::unistd::read(fd.as_raw_fd(), buf).map_err(std::io::Error::from)
            })
            .await
        }

        pub async fn async_fd_write<T: AsFd + AsRawFd>(
            afd: &AsyncFd<T>,
            buf: &[u8],
        ) -> io::Result<usize> {
            afd.async_io(Interest::WRITABLE, |fd| {
                nix::unistd::write(fd, buf).map_err(std::io::Error::from)
            })
            .await
        }

        pub async fn async_fd_read_vectored<T: AsFd + AsRawFd>(
            afd: &AsyncFd<T>,
            bufs: &mut [IoSliceMut<'_>],
        ) -> io::Result<usize> {
            afd.async_io(Interest::READABLE, |fd| {
                nix::sys::uio::readv(fd, bufs).map_err(std::io::Error::from)
            })
            .await
        }

        pub async fn async_fd_write_vectored<T: AsFd + AsRawFd>(
            afd: &AsyncFd<T>,
            bufs: &[IoSlice<'_>],
        ) -> io::Result<usize> {
            afd.async_io(Interest::WRITABLE, |fd| {
                nix::sys::uio::writev(fd, bufs).map_err(std::io::Error::from)
            })
            .await
        }
...
pub mod net {
    ...
    pub trait SocketExt {
        ...
        fn try_recv_multiple_vectored_from(
            &self,
            iovs: &mut [&mut [IoSliceMut<'_>]],
            msgs_out: &mut Vec<(usize, bool, SocketAddr)>,
        ) -> io::Result<usize>;
        ...
        fn try_send_multiple_vectored_to(
            &self,
            msgs: &[(&[IoSlice<'_>], Option<SocketAddr>)],
            res_out: &mut Vec<usize>,
        ) -> io::Result<usize>;
    }
    ...
    fn sockaddr_storage_as_socket_addr(sas: &SockaddrStorage) -> Option<SocketAddr> {
        match sas.family()? {
            nix::sys::socket::AddressFamily::Inet => {
                Some(SocketAddr::V4((*sas.as_sockaddr_in()?).into()))
            }
            nix::sys::socket::AddressFamily::Inet6 => {
                Some(SocketAddr::V6((*sas.as_sockaddr_in6()?).into()))
            }
            _ => None,
        }
    }
    ...
    fn recv_msg_as_result_tuple(rm: &RecvMsg<SockaddrStorage>) -> (usize, bool, SocketAddr) {
        (
            ...
            rm.address
                .and_then(|sa| sockaddr_storage_as_socket_addr(&sa))
                .unwrap_or(DUMMY_ADDR),
        )
    }
    ...
    fn recvmmsg_inner(
        ...
        match nix::sys::socket::recvmmsg(
            ...
            // not be equal
            unsafe {
                std::mem::transmute::<&mut [&mut [IoSliceMut<'_>]], &mut [&mut [IoSliceMut<'_>]]>(
                    iovs,
                )
            },
            MsgFlags::empty(),
    ...
    fn sendmmsg_inner(
        ...
        let addrs: Vec<_> = msgs
            .iter()
            .map(|(_iov, addr)| addr.map(SockaddrStorage::from))
            .collect(); // FIXME: can we avoid this allocation?
        match nix::sys::socket::sendmmsg(
            sock,
            &mut headers,
            msgs.iter().map(|(iov, _addr)| iov),
            addrs,
            &[],
            MsgFlags::empty(),
        ) {
    ...
    pub async fn udp_socket_recv_vectored_from(
        ...
        self_
            .async_io(tokio::io::Interest::READABLE, || recvmsg_inner(rfd, iovs))
            .await
    }
    ...
    pub async fn udp_socket_send_multiple_vectored_to(
        ...
        self_
            .async_io(tokio::io::Interest::WRITABLE, || {
                sendmmsg_inner(rfd, msgs, res_out)
            })
            .await
    }
    ...
    impl SocketExt for UdpSocket {
        ...
        fn try_recv_multiple_vectored_from(
            ...
            self.try_io(tokio::io::Interest::READABLE, || {
                recvmmsg_inner(rfd, iovs, msgs_out)
            })
        }
