...
pub const ZDP_KM_DATA_OFFSET: usize = ZDP_KM_HDR_OFFSET + std::mem::size_of::<ZdpKeyManagementHeader>();
...
pub const ZDP_REPORT_DATA_OFFSET: usize = ZDP_REPORT_HDR_OFFSET + std::mem::size_of::<ZdpReportHeader>();
...
pub fn build_zdp_report_packet<'buf>(pbuf: &'buf mut [u8; config::PACKET_BUFFER_SIZE], report_data: &[u8]) -> Packet<'buf> {
...
pub fn build_zdp_km_noise_packet<'buf>(pbuf: &'buf mut [u8; config::PACKET_BUFFER_SIZE], km_payload: &[u8]) -> Packet<'buf> {
    let mut pkt = Packet::new(pbuf, HEADROOM);
    pkt.put(&km_payload[..]);
...
pub fn parse_zdp_report_pkt(pkt: &Packet) -> Result<String, String> {
    ...
    if zpi_hdr.is_none() {
        return Err(String::from("error parsing ZPI header from decrypted payload"));
    }
    let zdp_hdr = ZdpBaseHeader::ref_from_prefix(&pkt.body()[ZDP_BASE_HEADER_OFFSET..]);
    if zdp_hdr.is_none() {
        return Err(String::from("parse report msg - error parsing ZDP header from decrypted payload"));
    }
    let zdp_hdr = zdp_hdr.unwrap();
    if zdp_hdr.packet_type != ZdpPacketType::Report {
        return Err(String::from(format!("parse report msg - expected REPORT packet, got {:?}", zdp_hdr.packet_type)));
    }
    let report_hdr = ZdpReportHeader::ref_from_prefix(&pkt.body()[ZDP_REPORT_HDR_OFFSET..]);
    if report_hdr.is_none() {
        return Err(String::from("parse report msg - error parsing REPORT header from decrypted payload"));
    }
    ...
    if ZDP_REPORT_DATA_OFFSET + strlen > pkt.body().len() {
        return Err(String::from("parse report msg - report data length exceeds packet length"));
    }
    match std::str::from_utf8(&pkt.body()[ZDP_REPORT_DATA_OFFSET..ZDP_REPORT_DATA_OFFSET+strlen]) {
        ...
        Err(e) => {
            return Err(String::from(format!("parse report msg - error parsing report data: {:?}", e)));
        }
    }
}
...
pub fn parse_km_payload<'buf>(msg_buf: &'buf [u8]) -> Result<&'buf [u8], String> {
    ...
    if zdp_hdr.is_none() {
        return Err(String::from("zdp/server - error parsing ZDP header from ZPI=0 message"));
    }
    let zdp_hdr = zdp_hdr.unwrap();
    if zdp_hdr.packet_type != ZdpPacketType::KeyManagement {
        return Err(String::from(format!("zdp/server - expected KM packet, got {:?}", zdp_hdr.packet_type)));
    }
    let km_hdr = ZdpKeyManagementHeader::ref_from_prefix(&msg_buf[ZDP_KM_HDR_OFFSET..]);
    if km_hdr.is_none() {
        return Err(String::from("zdp/server - error parsing KM header from ZPI=0 message"));
    }
    let km_hdr = km_hdr.unwrap();
    if !km_hdr.is_noise() {
        return Err(String::from(format!("zdp/server - expected NOISE KM message, got {:?}", km_hdr.message_type.get())));
    }
    let km_msg_len = usize::from(km_hdr.message_length);
    if msg_buf.len() < ZDP_KM_DATA_OFFSET + km_msg_len {
        return Err(String::from(format!("zdp/server - KM message truncated: expected {} got {}", ZDP_KM_DATA_OFFSET + km_msg_len, msg_buf.len())));
    }
